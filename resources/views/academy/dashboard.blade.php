...
{{-- HERO --}}
<div style="background:linear-gradient(135deg,#EEF2FF,#E0E7FF);border:1px solid #c7d2fe;border-radius:var(--radius);padding:28px 32px">
  <div style="font-size:13px;color:#2563EB;font-weight:500;margin-bottom:6px">👋 Welcome back</div>
  <h1 style="font-family:'DM Serif Display',serif;font-size:30px;font-weight:400;color:var(--ink);margin-bottom:8px">{{ auth()->user()->name }}</h1>
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px">
  <div class="card card-body">
    ...
    <div style="font-family:'DM Serif Display',serif;font-size:30px;color:var(--ink);line-height:1">{{ $totalCompleted }}</div>
  </div>
  <div class="card card-body">
    ...
    <div style="font-family:'DM Serif Display',serif;font-size:30px;color:var(--ink);line-height:1">{{ $hours }}h</div>
  </div>
  <div class="card card-body">
    ...
    <div style="font-family:'DM Serif Display',serif;font-size:30px;color:var(--ink);line-height:1">{{ $packages->count() }}</div>
  </div>
  <div class="card card-body">
      ...
      @foreach($packages as $package)
        ...
        <div class="card" style="margin-bottom:10px;overflow:hidden">
          <div style="display:flex;align-items:center;gap:14px;padding:16px">
            <div style="width:52px;height:52px;border-radius:10px;background:linear-gradient(135deg,#EEF2FF,#E0E7FF);display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0">{{ $package->emoji_icon }}</div>
      ...
      @foreach($packages as $package)
        ...
        <div class="card card-body" style="margin-bottom:10px">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
            ...
            <span style="margin-left:auto;font-family:monospace;font-size:13px;font-weight:600;color:var(--accent)">{{ $prog['percent'] }}%</span>
          </div>
          <div class="progress-track">
            <div class="progress-fill progress-fill-blue" style="width:{{ $prog['percent'] }}%"></div>
          </div>
