...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --bg: #F5F4F0; --bg2: #EEECEA; --bg3: #E8E6E1;
      --teal: #1a3a5c; --teal2: #0891b2;
      --teal-glow: rgba(8,145,178,0.08); --teal-border: rgba(26,58,92,0.2);
      --white: #1a1a2e; --white2: rgba(26,26,46,0.65); --white3: rgba(26,26,46,0.4);
      --border: rgba(26,26,46,0.1); --card: rgba(255,255,255,0.7);
    }
    html { scroll-behavior: smooth; }
    body { font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg); color: var(--white); overflow-x: hidden; }
    nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; display: flex; align-items: center; justify-content: space-between; padding: 18px 56px; border-bottom: 1px solid var(--border); background: rgba(245,244,240,0.92); backdrop-filter: blur(16px); }
    .nav-logo { display: flex; align-items: center; gap: 10px; text-decoration: none; }
    .logo-mark { width: 32px; height: 32px; border-radius: 8px; background: linear-gradient(135deg, var(--teal), #0891b2); display: flex; align-items: center; justify-content: center; font-size: 16px; font-weight: 800; color: #fff; font-family: 'DM Serif Display', serif; }
    .logo-text { font-family: 'Plus Jakarta Sans', sans-serif; font-size: 15px; font-weight: 700; color: #1a1a2e; }
    .logo-text span { color: #2563EB; }
    .nav-links { display: flex; align-items: center; gap: 28px; }
    .nav-links a { font-size: 13px; color: var(--white3); text-decoration: none; transition: color 0.2s; }
    .nav-links a:hover { color: var(--white); }
    .nav-cta { background: #2563EB !important; color: #fff !important; padding: 9px 20px; border-radius: 6px; font-weight: 600 !important; }
    ...
    .hero-glow { position: absolute; top: 20%; left: 50%; transform: translateX(-50%); width: 700px; height: 400px; border-radius: 50%; background: radial-gradient(ellipse, rgba(8,145,178,0.08) 0%, transparent 70%); pointer-events: none; }
    .hero-grid { position: absolute; inset: 0; opacity: 0.04; background-image: linear-gradient(rgba(26,26,46,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(26,26,46,0.06) 1px, transparent 1px); background-size: 60px 60px; }
    ...
    h1 { font-family: 'DM Serif Display', serif; font-size: clamp(44px, 7vw, 88px); font-weight: 800; line-height: 1.0; letter-spacing: -0.03em; color: var(--white); margin-bottom: 22px; animation: fadeUp 0.7s 0.1s ease both; }
    ...
    .btn-primary { background: var(--teal); color: #fff; padding: 13px 28px; border-radius: 7px; font-size: 14px; font-weight: 600; text-decoration: none; transition: all 0.2s; }
    ...
    .stat-num { font-family: 'DM Serif Display', serif; font-size: 32px; font-weight: 800; color: var(--teal); letter-spacing: -0.02em; }
    ...
    h2 { font-family: 'DM Serif Display', serif; font-size: clamp(32px, 4vw, 52px); font-weight: 800; line-height: 1.05; letter-spacing: -0.03em; color: var(--white); margin-bottom: 16px; }
    ...
    .fw-title { font-family: 'DM Serif Display', serif; font-size: 18px; font-weight: 700; color: var(--white); margin-bottom: 10px; }
    ...
    .course-card { background: #ffffff; padding: 32px; position: relative; transition: background 0.2s; }
    .course-card:hover { background: var(--bg3); }
    .course-card.featured { grid-column: span 2; }
    .course-badge { position: absolute; top: 20px; right: 20px; background: #0891b2; color: #fff; font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; padding: 4px 10px; border-radius: 100px; }
    ...
    .course-title { font-family: 'DM Serif Display', serif; font-size: 20px; font-weight: 700; color: var(--white); margin-bottom: 10px; letter-spacing: -0.02em; line-height: 1.2; }
    ...
    .course-price { font-family: 'DM Serif Display', serif; font-size: 26px; font-weight: 800; color: var(--white); letter-spacing: -0.03em; }
    ...
    .btn-sm { background: #0891b2; color: #fff; padding: 9px 18px; border-radius: 6px; font-size: 12px; font-weight: 600; text-decoration: none; white-space: nowrap; transition: background 0.2s; }
    ...
    .cta-title { font-family: 'DM Serif Display', serif; font-size: 38px; font-weight: 800; color: var(--white); line-height: 1.1; letter-spacing: -0.03em; }
    ...
    .footer-logo-text { font-family: 'DM Serif Display', serif; font-size: 14px; font-weight: 700; color: var(--white); }
...
<nav>
  <a href="https://www.peopleflow.io" class="nav-logo">
    <img src="/images/peopleflow-logo.png" alt="Peopleflow" style="height:28px;width:auto;"> <span style="font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:700;color:#2563EB;margin-left:4px">Academy</span>
  </a>
  <div class="nav-links">
  ...
  @foreach($packages as $index => $package)
    <div class="course-card {{ $index === 0 ? 'featured' : '' }}">
      ...
      <div class="course-meta" style="flex-direction:column;align-items:flex-start;gap:14px">
        ...
        <a href="{{ auth()->check() ? route('academy.checkout', $package->slug) : route('register') . '?intended=' . urlencode(route('academy.checkout', $package->slug)) }}" class="btn-sm">Enroll Now →</a>
      </div>
    </div>
  @endforeach
  </div>
